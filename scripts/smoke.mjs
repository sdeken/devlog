...
try {
  ...
  check((await page.locator('.hit-day').first().textContent()).startsWith('Website'), 'search results name the page')
  ...
  check(acmeTotal === '2', `client row sums its notes for the week (${acmeTotal})`)
  ...
  await page.locator('.review-note-link').last().click()
  await page.waitForSelector('.composer-new .composer-editor', { timeout: 10_000 })
  check(true, 'clicking a review note opens its page')
  await page.locator('.page-journal').click()
  await page.waitForSelector('.page-head h2:has-text("Journal")')
