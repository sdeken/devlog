...
describe('DevlogStore', () => {
  ...
  it('searches across days, newest first', async () => {
    ...
    const { notes } = await store.search('alpha')
    expect(notes.map((h) => h.entry.markdown)).toEqual(['ALPHA two', 'alpha one'])
    expect(notes.every((h) => h.pageId === 'journal')).toBe(true)
  })
...
describe('pages in the store', () => {
  ...
  it('searches across pages', async () => {
    ...
    const { notes, wikis } = await store.search('needle')
    expect(notes.map((h) => `${h.pageId}:${h.entry.markdown}:${h.archived}`)).toEqual(['journal:needle in journal:false', 'acme:needle on page:false'])
    expect(wikis).toEqual([])
  })

  it('archives pages and whole categories, keeps them searchable, and restores them', async () => {
    const web = await store.createPage({ title: 'Website', category: 'Acme Corp / Web' })
    const gen = await store.createPage({ title: 'General', category: 'Acme Corp' })
    const other = await store.createPage({ title: 'Site', category: 'Globex / Web' })
    await store.addEntry(web.id, 'archived needle', {}, new Date(2026, 8, 19, 9))

    const archived = await store.setPageArchived(web.id, true)
    expect(archived.archived).toBe(true)
    expect(await fs.readFile(path.join(root, `pages/${web.id}/page.md`), 'utf8')).toContain('archived: true')
    expect((await store.listPages()).find((p) => p.id === web.id)?.archived).toBe(true)
    const hits = await store.search('archived needle')
    expect(hits.notes.map((h) => [h.pageId, h.archived])).toEqual([[web.id, true]])
    expect((await store.setPageArchived(web.id, false)).archived).toBe(false)

    const result = await store.setCategoryArchived(['acme corp'], true)
    expect(result.pages).toBe(2)
    const pages = await store.listPages()
    expect(pages.find((p) => p.id === web.id)?.archived).toBe(true)
    expect(pages.find((p) => p.id === gen.id)?.archived).toBe(true)
    expect(pages.find((p) => p.id === other.id)?.archived).toBe(false)
    expect((await store.readWiki(['Acme Corp'])).archived).toBe(true)
    expect((await store.listWikis()).map((w) => [w.path.join('/'), w.archived])).toEqual([['acme corp', true]])

    const restored = await store.setCategoryArchived(['Acme Corp'], false)
    expect(restored.pages).toBe(2)
    expect((await store.listPages()).filter((p) => p.archived)).toHaveLength(0)
    expect((await store.readWiki(['Acme Corp'])).archived).toBe(false)
    await expect(store.setPageArchived('journal', true)).rejects.toThrow(/journal/)
  })

  it('stores category wikis with assets and finds them in search', async () => {
    expect(await store.readWiki(['Acme Corp', 'Web'])).toMatchObject({ path: ['Acme Corp', 'Web'], exists: false, markdown: '' })
    const asset = await store.saveWikiAsset(['Acme Corp', 'Web'], new Uint8Array([1, 2, 3]), 'image/png', undefined, new Date(2026, 8, 19, 9, 0, 0))
    expect(asset.src).toMatch(/^categories\/acme-corp\/web\/assets\/2026-09-19-090000-/)
    const wiki = await store.writeWiki(['Acme Corp', 'Web'], `# Links\n\nTracker: https://issues.example\n\n![diagram](${asset.src})\n`, new Date(2026, 8, 19, 10))
    expect(wiki).toMatchObject({ path: ['Acme Corp', 'Web'], exists: true, archived: false, updatedAt: new Date(2026, 8, 19, 10).toISOString() })
    const file = await fs.readFile(path.join(root, 'categories/acme-corp/web/wiki.md'), 'utf8')
    expect(file).toContain('path: Acme Corp / Web')
    expect(file).toContain('![diagram](assets/2026-09-19-090000-')
    const back = await store.readWiki(['acme corp', 'web'])
    expect(back.markdown).toContain(`![diagram](${asset.src})`)
    expect(back.path).toEqual(['Acme Corp', 'Web'])
    expect((await store.listWikis()).map((w) => w.path)).toEqual([['Acme Corp', 'Web']])
    const { wikis } = await store.search('issues.example')
    expect(wikis).toHaveLength(1)
    expect(wikis[0]).toMatchObject({ path: ['Acme Corp', 'Web'], archived: false })
    expect(wikis[0].excerpt).toContain('issues.example')
  })
})
